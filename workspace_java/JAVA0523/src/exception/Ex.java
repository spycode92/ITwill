...
public class Ex {
	...
	public static void main(String[] args) {
		 ...
		 * - 오류(Error)와 달리 심각도 낮으며, 예외 처리(Exception Handling)를 통해
		 *   예외 발생 시 해결책을 기술하여 비정상적인 종료를 막을 수 있음.
		 * - 예외 처리를 위해 try ~ catch 문을 사용하여 처리
		 *   => 예외가 발생할 것으로 예상되는 코드들을 try 블록 내에 기술하여,
		 ...
		 * - 컴파일 시점에서 발생 여부를 체크하는 Compile Checked Exception 과
		 ...
		 * 		// => 예외가 발생하면 코드 아래쪽 나머지 코드는 실행되지 못함
		 * } catch(예외클래스명1 변수명) {
...
//		System.out.println(str.length());
		...
		// 예외 처리
		// try ~ catch 단축키 : Alt + Shift + Z -> Try/catch Block 선택
		
		System.out.println("프로그램 시작!");
		
		try {
			int num = 1;
			System.out.println(3 / num); // ArithmeticException 예외 발생
...
//			System.out.println("나머지 모든 예외 처리 - " e.getMessage());
			...
			// => if문과 마찬가지로 순차적으로 수행되므로
			//    하위클래스 타입부터 상위클래스 타입으로 예외 처리 필수!
