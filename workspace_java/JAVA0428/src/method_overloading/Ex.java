...
public class Ex {
	...
	public static void main(String[] args) {
		 ...
		 * - 동일한 이름의 파라미터가 다른 메서드를 여러 번 정의한 것
		 * - 비슷한 기능을 수행하지만, 전달받는 데이터의 타입이 다른 경우
		 *   메서드 이름을 각각 따로 정의하지 않고, 동일한 이름으로
		 *   파라미터만으로 구분되는 메서드를 여러 개 정의한 넋
...
class OverloadingMethod {
	...
	// => 동일한 이름의 파라미터 타입만 다른 서로 다른 메서드를 여러 번 정의
	// 1. 정수 2개를 전달받아 덧셈 수행
...
class NormalMethod {
	...
	// => 메서드 정의 시 메서드명을 달리 해야하므로 호출 시 구분이 불편할 수 있음
