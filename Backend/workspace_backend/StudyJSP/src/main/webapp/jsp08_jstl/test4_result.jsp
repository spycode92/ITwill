...
<body>
	...
	<h3>
		<%-- EL 활용하여 파라미터 name, age값, 세션 객체의 id 속성값 출력 --%>
		이름 : ${param.name } <br>
		나이 : ${param.age }<br>
...
<%-- 		취미 : ${paramValues.hobby[0]} / ${paramValues.hobby[1]} / ${paramValues.hobby[2]} --%>
	...
	<hr>
	<%
	// 자바의 일반 for문 활용하여 제어변수 i값이 1 ~ 10 까지 1식 증가하면서 i 값 출력
// 	for(int i = 1; i <= 10; i++){
// 		out.print(i);
// 	}
	%>
	...
	<%-- 1 ~ 10 까지 1씩 증가하는 i 값 --%>
	<c:forEach var="i" begin="1" end="10" ><%-- step속성 생략시 1씩 증가 --%>
		${ i}
	</c:forEach>
	<br>
...
// 	2) 배열 크기만큼 일반 for문으로 반복 접근하여 데이터 출력
	...
	<%-- 0 ~ 2 까지 1씩 증가하면서 배열 접근 배열 인덱스값 지정--%>
	...
	<%-- end 속성값으로 배열 크기르 ㄹ알아내는 length() 함수를 EL로 표현 --%>
	<c:forEach var="i" begin="0" end="${fn:length(paramValues.hobby) - 1}" >
		${paramValues.hobby[i] }
	</c:forEach>
	<hr>
	
	<%-- 반복되는 인덱스 값을 확인하려면 varStatus 속성을 통해 count 또는 index 값 혹인 가능 --%>
...
<%-- <c:set> 태그를 통해 함수 처리 결과값을 저장한 뒤 사용도 가능함 --%>
	...
	// page 여역에 "names" 라는 속성명으로 names 배열 저장
	pageContext.setAttribute("names", names);
	%>
	<br>
	<%-- <c:forEach> 태그로 동일한 작업 수행 --%>
	<c:forEach var="name" items="${pageScope.names }" end="${fn:length(pageScope.names) - 1}" > 
		<c:choose>
