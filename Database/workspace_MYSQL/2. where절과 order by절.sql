...
-- [비교연산자3] like
-- 패턴 일치 여부를 비교하는 연산자
-- like 연산자와 함께 사용되는 기호 1) % : 0개 또는 그이상의 문자가 올 수 있음. 
--                          2) _ : 반드시 1개의 문자가 와야함.
-- a로 시작되는 문자열 : 'a%'
-- a가 포함된 문자열 : '%a%'
-- a로 끝나는 문자열 : %a
...
-- and 와 or가 함께 사용된 경우 and가 or보다 우선순위가 높다.
-- 우선순위를 지정하고자 할 경우 괄호 사용해야 함.
select employee_id, last_name, salary, department_id
from employees
...
-- 2. employees 테이블로부터 커미션을 받지 않는 모든 사원의 last_name, salary, commission_pct를 출력하되
--  salary를 기준으로 내림차순 정렬하시오.
